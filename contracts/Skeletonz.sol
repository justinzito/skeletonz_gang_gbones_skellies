...
contract Skeletonz is ERC721A, ReentrancyGuard, Ownable {
	
	uint public constant maxTokens = 1500;
    uint public total = 0;
	
	uint whitelistMintCost = 0.075 ether;
	uint publicMintCost = 0.08 ether;
	...
	constructor() ERC721A("Skeletonz", "Skeleton") Ownable() {

        whitelistMintStart = 1647181800; // 13. March 2022 - 14:30 UTC
        publicMintStart = 1647196200; // 13. March 2022 - 18:30 UTC (+4 hours)
	...
	function whitelistClaim(bytes32[] memory proof, uint256 quantity) public nonReentrant payable {
        require(whitelistMintStart < block.timestamp, "Whitelist mint not started yet");
        require(publicMintStart > block.timestamp, "Whitelist mint is over");
		require(quantity <= 2, "Maximum number of tokens is 2");
        require(total + quantity < maxTokens, "Mint over the max tokens limit"); // This will never happen, remove?
		require(msg.value == whitelistMintCost * quantity, "Incorrect mint cost value");
        ...
        // Set the _whitelistMinters to 1 as it has already minted
        _whitelistMinters[_msgSender()] = 1;
    }

	function publicMint(uint256 quantity) external nonReentrant payable {
		require(publicMintStart < block.timestamp, "Public mint not started yet");
        require(total + quantity < maxTokens, "Mint over the max tokens limit");
        require(quantity <= 5, "Maximum number of tokens is 5");
        require(msg.value == publicMintCost * quantity, "Incorrect mint cost value");
